...
_attrs_for_upgradable_github_archive = {
    "sha256": attr.string(
        doc = """The expected SHA-256 of the file downloaded.
...
def _impl_for_upgradable_github_archive(ctx):
    ...
    splitted_slug = ctx.attr.slug.split("/")
    if len(splitted_slug) != 2:
        fail("Bad GitHub slug: {}".format(ctx.attr.slug))
    ...
    # constraint = kwargs.get("constraint", "HEAD")
    commit = "dfcfe825005ffba4a7c15cd9ddac737d040d2506"
    archive_type = "zip"
    url = "https://github.com/{}/archive/{}.{}".format(ctx.attr.slug, commit, archive_type)
